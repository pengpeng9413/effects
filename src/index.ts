...
const loadImage = (
  ...
  return new Promise<HTMLImageElement | null>((resolve) => {
    if (IMAGE_CACHES.has(image)) {
      IMAGE_CACHES.get(image).setAttribute(
        "data-isContan",
        isContan ? "1" : "0"
      );
...
/** 图片滚动速度映射
 *  其余有其他规则通用规则  (speed-2)*60; 每帧移动的像素
 */
export const speedMap: Map<number, number> = new Map([
  [1, 20],
  [2, 30],
]);
...
const animate = function (
  ...
  const animateProps: AnimateProps = {
    ...
    speed,
    easing:
      typeof easing === "string" && Tween[easing]
        ? Tween[easing]
        : Tween.linear,
  };
...
export class Effect {
  ...
  static animate(
    ...
    loadImage(img, options.isContan).then((image) =>
